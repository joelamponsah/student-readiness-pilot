...
6. Confirm accuracy values stay in the expected range.
   - `full_test_accuracy = marks / max_marks_db` should stay between 0 and 1.
   - `attempted_accuracy = correct_answers / attempted_questions` should stay between 0 and 1 where question-result rows exist.
   - `accuracy_denominator_source` should normally be `max_marks_db`.
...
14. Confirm `no_of_questions` is treated as a DQ field, not a score denominator.
...
- Proxy percentages should reconcile to `marks / max_marks_db * 100`.
