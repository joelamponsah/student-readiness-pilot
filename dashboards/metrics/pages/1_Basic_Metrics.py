...
# Fallback for time_consumed
# Preferred formula: time_taken / test duration.
# Showcase fallback: time_taken / average time_taken.
...
with c2:
    if "time_consumed" in df.columns and df["time_consumed"].notna().any():
        fig2 = px.histogram(df, x="time_consumed", nbins=30, title="Time consumed distribution")
        st.plotly_chart(fig2, use_container_width=True)
    else:
        st.info("No `time_consumed` column available.")

if "speed_raw" in df.columns and df["speed_raw"].notna().any():
    fig3 = px.histogram(df, x="speed_raw", nbins=30, title="Raw speed distribution")
...
with c4:
    if "accurate_speed" in df.columns and df["accurate_speed"].notna().any():
        fig4 = px.histogram(df, x="accurate_speed", nbins=50, title="Adjusted speed distribution")
...
with c5:
    if "efficiency_ratio" in df.columns and df["efficiency_ratio"].notna().any():
        fig5 = px.histogram(df, x="efficiency_ratio", nbins=50, title="Efficiency ratio distribution")
...
# ------------------------------------------------
# Dataset average interpretation, repeated near averages
# ------------------------------------------------
with st.expander("Click for plain-English explanation of dataset averages", expanded=False):
    st.info(
        """
**Dataset average interpretation**

The average accuracy is **0.574**, or about **57.4%**.  
This suggests a **moderate baseline**. Learners are not failing badly overall, but the average is not high enough to claim strong exam readiness.

The average adjusted speed is **2.115**, meaning learners answer about **2.1 correct answers per minute** on average.  
This is useful, but speed alone does not prove readiness.

The average time taken is about **50 minutes per attempt**.  
This gives a rough baseline, but raw time values include outliers, so time should be interpreted carefully.

The average time consumed is **1.000** because this showcase uses normalized time.  
A value of 1.0 represents the typical attempt baseline.

The average efficiency is **2.584**.  
This suggests learners are producing some correct work for the time used, but efficiency should be reviewed together with accuracy, pass rate, and consistency.

**Simple takeaway:**  
The dataset shows useful learning activity and a moderate readiness baseline, but further model validation is needed before using these metrics for final intervention decisions.
"""
    )
...
if "accuracy_total" in df.columns and df["accuracy_total"].notna().any():
    ...
    with c6:
        fig6 = px.histogram(df, x="rel_acc", nbins=30, title="Relative accuracy distribution")
        st.plotly_chart(fig6, use_container_width=True)

    with c7:
        if "speed_rel_time" in df.columns and df["speed_rel_time"].notna().any():
            fig7 = px.histogram(df, x="speed_rel_time", nbins=30, title="Relative time / speed distribution")
...
# Main headline variability metrics
# Raw time std is intentionally not shown as a headline because time outliers distort it.
col1, col2, col3 = st.columns(3)

col1.metric("Std accuracy", fmt_num(std_accuracy, 3))
col2.metric("Std adjusted speed", fmt_num(std_adj_speed, 3))
col3.metric("Std efficiency", fmt_num(std_efficiency, 3))
...
with st.expander("Raw time outlier diagnostic", expanded=False):
    st.warning(
        f"""
**Raw time metrics are shown here as diagnostics, not headline readiness indicators.**

- Raw std time taken: **{fmt_num(std_time_taken, 3)}**
- Raw std time consumed: **{fmt_num(std_time_consumed, 3)}**

These values are very high because the raw time fields contain outliers.  
This supports the need for stronger data-quality rules and robust time handling in the next development phase.
"""
    )
