...
if df is None:
   ...
   # with c3:
    ...
    c4, c5 = st.columns(2)
    with c4:
        fig4 = px.histogram(df, x='accurate_speed', nbins=5, title='Correct Answers by Time Taken Distribution')
        st.plotly_chart(fig4, use_container_width=True)
    with c5:
        fig5 = px.histogram(df, x='efficiency_ratio', nbins=5, title='Efficieny - Accuracy by time Consumption')
