...
The DQ-critical pages now mostly follow the updated bootstrap direction: published rollups use completed-only, deduped, strict-pass-mark policy; learner drilldown and proxy-sequence views may include `unknown_but_usable` rows as diagnostic evidence, but they must remain explicitly labeled.
...
2. The user summary page intentionally supports a diagnostic mode where `unknown_but_usable` can be included, but this needs stronger visible labeling around partial evidence.
